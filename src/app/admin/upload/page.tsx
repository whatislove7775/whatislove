...
export default function AdminUploadPage() {
  ...
  const upload = async () => {
    ...
    for (const { file } of files) {
      ...
      const res = await fetch('/api/upload-image', { method: 'POST', headers: ah(), body: formData });
  ...
  return (
    <div style={{ maxWidth: '600px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ fontWeight: 800, fontSize: '18px' }}>загрузка фото</div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <div style={{ fontWeight: 800, fontSize: '13px' }}>папка в storage:</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          {FOLDERS.map(f => (
            <button key={f} onClick={() => setFolder(f)} style={{ fontFamily: 'inherit', fontSize: '13px', fontWeight: 800, padding: '4px 10px', border: '1.5px solid #000', background: folder === f ? '#000' : '#fff', color: folder === f ? '#fff' : '#000', cursor: 'pointer' }}>
              {f}
            </button>
          ))}
        </div>
        <input
          ...
          style={{ padding: '8px 12px', border: '1px solid #ccc', fontFamily: 'inherit', fontSize: '13px', width: '100%', boxSizing: 'border-box', outline: 'none' }}
        />
      </div>
      ...
      <div
        ...
        style={{ border: `2px dashed ${dragging ? '#000' : '#ccc'}`, padding: '40px 20px', textAlign: 'center', cursor: 'pointer', fontWeight: 800, fontSize: '14px', background: dragging ? '#f5f5f5' : '#fff' }}
      >
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
          {files.map(({ file, preview }, i) => (
            <div key={i} style={{ position: 'relative', width: '100px' }}>
              <img src={preview} style={{ width: '100px', height: '100px', objectFit: 'cover', display: 'block' }} alt="" />
              {file.size > MAX_BYTES && (
              ...
              <div style={{ fontSize: '10px', marginTop: '2px', textAlign: 'center' }}>{(file.size / 1024).toFixed(0)} KB</div>
            </div>
          ))}
        </div>
      )}
      ...
      {files.length > 0 && (
        <button onClick={upload} disabled={uploading} style={{ fontFamily: 'inherit', fontWeight: 800, fontSize: '15px', padding: '12px', border: '2px solid #000', background: uploading ? '#f5f5f5' : '#000', color: uploading ? '#999' : '#fff', cursor: uploading ? 'not-allowed' : 'pointer' }}>
      ...
      {results.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <div style={{ fontWeight: 800, fontSize: '13px' }}>готово — скопируй url:</div>
          {results.map((r, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '4px', padding: '12px', border: '1px solid #e0e0e0', background: '#fafafa' }}>
              ...
              <div
                ...
                style={{ fontSize: '12px', wordBreak: 'break-all', cursor: 'pointer', padding: '6px 8px', background: '#fff', border: '1px solid #ddd' }}
                title="нажми чтобы скопировать"
              >
                {r.url}
              </div>
              <div style={{ fontSize: '11px', color: '#999' }}>нажми чтобы скопировать</div>
            </div>
          ))}
        </div>
      )}
